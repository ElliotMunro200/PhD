...
def train(num_iterations, agent, env, env_name, evaluate, validate_steps, output, framerate, use_wandb, make_animation,
    ...
    while step < num_iterations:  # total number of training iterations/steps
        ...
        step_type, reward, discount, obs = env.step(action_env)
        next_state = ([obs['position'][0], obs['position'][2], obs['velocity'][0], obs['velocity'][1]]) # extraction.
        if reward == None: # so that rewards can be summed.
            reward = 0.0

        # determining if this new state requires an episode reset.
        done = env._reset_next_step  # if the episode steps surpass the max episode length (calculated from time limit),
        if abs(next_state[1]) >= 0.5 * np.sqrt(3):  # or if pole gets below 30 deg from vertical on either side (x,v,t,w):
            done = True  # reset the episode next step.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--discount', default=0.99, type=float, help='discounting of future rewards in update')
    ...
    parser.add_argument('--max_episode_length', default=None, type=int, help='max number of steps in an episode')
    ...
    if args.use_wandb:
        run = wandb.init(
            project="S_L_" + args.env,  # name of project on WandB
            config=args,
