...
def train(num_iterations, agent, env, env_name, evaluate, validate_steps, output, framerate, max_episode_length=None, debug=False):
    ...
    while step < num_iterations: # total number of training iterations/steps
        ...
        if episode % 10 == 0:
            if len(frames) < env._physics.data.time * framerate + (episode//10)*max_episode_length*env._physics.timestep():
                print(env._physics.data.time)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_episode_length', default=10, type=int, help='max number of steps in an episode')
