...
def train_CartPole_DQNet(env, env_name, model, replay_buffer, batch_size, optimizer, num_iterations,
    ...
    for frame_idx in range(1, num_iterations + 1):
        ...
        # after-step model update and loss logging
        if len(replay_buffer) > batch_size:
            ...
            if use_wandb and frame_idx % (num_iterations/100) == 0:
                metric_name = "loss"
                train_log(metric_name, loss, frame_idx)

        # end of episode reward logging and resetting
        if done:
            ...
            episode_reward = 0
            episode += 1
            # no state=env.reset() needed, because done automatically through the 'TimeStep'.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ep_time_limit', default=1, type=int, help='episode time limit in seconds')
