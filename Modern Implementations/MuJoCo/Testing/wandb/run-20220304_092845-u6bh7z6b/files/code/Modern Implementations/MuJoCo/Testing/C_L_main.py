...
def train(num_iterations, agent, env, env_name, evaluate, validate_steps, output, framerate, max_episode_length=None, debug=False):
    ...
    while step < num_iterations: # total number of training iterations/steps
        if step % int(num_iterations/max(num_iterations,max_episode_length)) == 0:
            print(step)
        ...
        if step > args.warmup:
            ...
            if step % (num_iterations/max_episode_length) == 0:
                metric_name = "loss"
...
if __name__ == "__main__":
    ...
    num_goals = 1
    num_states = num_states_qpos + num_states_qvel + num_goals
