...
def train(num_iterations, agent, env, env_name, evaluate, validate_steps, output, framerate, max_episode_length=None, debug=False):
    ...
    wandb.watch(agent, log="all", log_freq=10)  # "Hooks into the torch model (DQN) to collect gradients and the topology."
    frames = []
    losses = []
    all_rewards = []
    ##############################################
...
if __name__ == "__main__":
    ...
    run = wandb.init(
        project="C_L" + args.env,  # name of project on WandB
        config=args,
